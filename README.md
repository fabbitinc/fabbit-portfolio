...
AI 코딩 에이전트(Claude Code)와 FastAPI로 개발을 시작했습니다. 1만 줄까지는 컨벤션 문서와 프롬프트로 통제됐지만, AI용 스킬과 linter/hook을 붙였음에도 3만 줄을 넘어서자 **linter를 우회하는 패턴, 로직 분산, 동일한 버그의 반복**이 누적됐습니다. 원인은 분명했습니다 — 규칙이 권고(advisory)에 머무는 한, 에이전트는 결국 가장 쉬운 경로로 그것을 우회합니다.

**판단 — Context가 아니라 Harness가 필요하다.**
규칙을 "알려주는" 수준(Context)과 "어기면 빌드를 멈추는" 수준(Harness)은 근본적으로 다릅니다. 동적 타입·런타임 검증 위주의 FastAPI로는 이 강제력을 코드로 표현하기 어려웠습니다. 그래서 **강제력을 코드로 표현할 수 있는 환경** — Java 21 타입 시스템 + Spring 프레임워크 제약 + ArchUnit — 으로 마이그레이션했습니다.

**결과 — 3겹의 하네스.**
AI가 코드를 생성하는 경로 위에 세 개 층의 방어선을 직접 설계했습니다. 앞 층은 우회될 수 있어도, 마지막 층은 우회되지 않습니다.

- **① 스킬 (Context 주입)** — DDD 레이어마다 하나씩 총 9개의 컨벤션 스킬(SKILL.md)을 작성해, 네이밍·입출력 모델(`*Command/*Result`)·트랜잭션 경계·도메인 간 호출(`*Api/*Policy` 경유) 규칙을 주입합니다.
- **② 라우팅 + 훅 (강제 실행)** — 레이어별 트리거에 해당하면 해당 스킬을 반드시 로드하게 하고("스킬 적용이 가능한데 사용하지 않는 행동을 금지"), 코드 변경 시 항상 `make lint`(→ `archTest`)와 전체 테스트를 실행하는 훅으로 검증 없이 작업이 끝나지 않게 했습니다.
- **③ ArchUnit 가드레일 (비우회 강제)** — 스킬과 훅을 우회하더라도 최종적으로 걸리는 방어선. 5개 테스트 클래스에 걸친 수십 개의 아키텍처 규칙이 매 빌드·CI에서 실행되며, 위반 시 빌드가 실패합니다.

<details>
<summary><b>ArchUnit / 소스 정적 분석 가드레일 — 전체 보기 (위반 시 빌드 실패)</b></summary>
...
**효과:** Java로 옮기며 코드량은 늘었지만, **에이전트가 수만 줄을 생성하는 동안에도 4계층 의존성·Rich Domain Model·서비스 패턴이 구조적으로 유지**됐습니다. "사람이 매 PR마다 리뷰로 잡아내는" 방식에서 "시스템이 항상 잡아내는" 방식으로 전환한 것이 이 프로젝트의 핵심 엔지니어링 성과입니다.
